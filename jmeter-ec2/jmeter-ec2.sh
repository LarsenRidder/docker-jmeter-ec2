...
function runcleanup() {
	...
	if [ ! -z "$LOADOSOPHIA_TOKEN_FILE" ] ; then
		echo "Adding csv column names"
		sed -i "1itimeStamp,elapsed,label,responseCode,responseMessage,threadName,dataType,success,bytes,grpThreads,allThreads,Latency,Hostname" $project_home/$project-$DATETIME-complete.jtl
		echo "Gzipping..."
		gzip $project_home/$project-$DATETIME-complete.jtl

		echo "Uploading results to $LOADOSOPHIA_PROJECTNAME Loadosophia's Project"
		curl https://loadosophia.org/api/file/upload/ -H "X-Loadosophia-Token: `cat $LOADOSOPHIA_TOKEN_FILE`" -F "projectKey=$LOADOSOPHIA_PROJECTNAME" -F "jtl_file=@$project_home/$project-$DATETIME-complete.jtl.gz"

		mv $project_home/$project-$DATETIME-complete.jtl.gz $project_home/results/
	else
